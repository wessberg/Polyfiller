...
function visitCssCalcExpression(expression, depth) {
	return depth === 0 ? `calc(${visitCssExpressions(expression.children, depth + 1)})` : `(${visitCssExpressions(expression.children, depth + 1)})`;
}
...
function parseCalc(input, offset = {pos: 0, end: input.length}) {
	...
	while (!isEOF()) {
		...
		if (!ignore && next === "c" && peek(++peekIndex) === "a" && peek(++peekIndex) === "l" && peek(++peekIndex) === "c" && peek(++peekIndex) === "(") {
			flushReadBuffer();
