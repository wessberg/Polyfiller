...
export class Server implements IServer {
	...
	public async serve (options: IServeOptions): Promise<void> {
		...
		const http2RequestHandler = async (request: HttpIncomingMessage|Http2ServerRequest, response: HttpServerResponse|Http2ServerResponse, tls: boolean) => this.onRequest(request, response, options, tls);
		...
		await new Promise<void>(resolve => this.server!.listen({
			port: options.port,
			path: options.root
		}, () => {

			const startMessage = options.http2
				? `${shouldRunSecureServer ? "Secure " : ""}HTTP2 `
				: `${shouldRunSecureServer ? "HTTPS " : "HTTP "}`;
